...
import { Moreproducts } from "./product/Moreproducts"
...
const ProductDetail = () => {
  ...
  return (
    <>
      ...
      {/* 內容 */}
      {contentLoading && (
        <div className="flex flex-col lg:flex-row animate-fadeIn mt-16 pb-14 dark:text-white">
          {/* 左邊 */}
          <div className="m-auto w-[90%] max-w-[990px] mr-5">
            <h1 className="border-l-[5px] border-lime-200 pl-4 mb-2">
              {productDeatil.title}
            </h1>
            <div className="text-[14px] text-zinc-500 mb-7 dark:text-white">{`上架時間：${new Date(productDeatil.time).getFullYear().toString()}.${(new Date(productDeatil.time).getMonth() + 1).toString()}.${new Date(productDeatil.time).getDate().toString()}`}</div>
            <div className="list-img pb-[60%] mb-5 rounded-[10px] overflow-hidden">
              <img src={`${productDeatil.imageUrl}`} />
            </div>
            {/* 編輯器內容 */}
            <div className="dark:text-white" dangerouslySetInnerHTML={{ __html: productDeatil.content }}></div>
            {/* 回上一頁*/}
            <button className="sumit-btn lg:w-44 mt-16" onClick={()=>window.history.go(-1)}>回上一頁</button>
         
          </div>
          {/* 右邊 */}
          <aside className="w-[90%]  lg:w-auto m-auto mt-12 lg:mb-0 lg:mt-0 dark:text-white">
            <div className="sticky top-5 border p-6 rounded-lg mb-2">
              <div className="">
                <div className="text-[18px] font-medium">
                  <span className="text-gray-400 mr-2">建議售價</span>
                  <span className="text-gray-400 line-through">
                  $NT {productDeatil.origin_price}
                  </span>
                  <div className="text-sm mt-3">
                  庫存 {productDeatil.unit} 雙
                  </div>
                </div>
                <div className="text-2xl mb-4">$NT {productDeatil.price}</div>
              </div>
              <div className="flex mb-3">
                ...
                <input
                  className=" rounded-none text-center border-[1px] p-1 dark:text-black"
                  type="number"
                  readOnly
                  value={cartQuantity}
                />
                <button
                  className="arrw-btn"
                  onClick={() => {
                    setCartQuantity((pre) => (pre >= productDeatil.unit ? pre : pre + 1))
                  }}
                >
                  <FiPlus />
                </button>
              ...
              {cartQuantity >= productDeatil.unit && <div className="mt-1 text-sm text-red-500">最多選購 {productDeatil.unit} 雙</div>}
            </div>
          </aside>
