    ...
    <div className="content-box">
      <div >
        ...
        <ul className="space-y-5">
          <span className="text-center block text-[#ccc] py-3" dangerouslySetInnerHTML={{ __html: isdefault }}></span>
          {productDetail.map((products) => {
            return (
              <li key={products.id} className="group relative">
                <Link className="flex border-b group-hover:bg-[#d9f99d] transition duration-300 p-2" to={`/products/${products.id}`}>
                  {/* img */}
        ...
        <div className="mt-10 flex flex-col lg:flex-row justify-end items-center">
          <div className="lg:w-[30%]">
            共計 <b className="text-[#3b3b3b] text-4xl">{totalQuantity}</b> 件 
            <span className=" ml-2 text-red-600 font-bold text-2xl">$NT ${ total}</span>
          </div>
